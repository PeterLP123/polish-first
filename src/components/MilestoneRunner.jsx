...
import { normalisePolish, scoreCloze, scoreForRating, scoreReading, scoreWriting } from "../lib/learning.js";
...
export default function MilestoneRunner({ milestone, onClose, onComplete, onAttempt }) {
  ...
  const completeTask = (score, rating = null) => {
    ...
    if (task.kind === "speaking") {
      ...
      setFinished({ mean, passed: mean >= 0.8 });
      return;
    }
  ...
  return <section className="milestone-runner panel" role="dialog" aria-modal="true" aria-label={`${milestone.title} milestone`}>
    ...
    </> : <div className="milestone-result"><span className="celebration">{finished.passed ? "🎉" : "🌱"}</span><h2>{finished.passed ? "Scenario ready" : "Keep building"}</h2><p>Your automatic score was <strong>{Math.round(finished.mean * 100)}%</strong>. Passing requires 80% plus the speaking self-check.</p><div><button className="secondary-button" onClick={retry}>Try again</button><button className="primary-button" onClick={onClose}>Back to insights</button></div></div>}
  </section>;
}
