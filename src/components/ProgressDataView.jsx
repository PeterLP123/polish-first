...
export default function ProgressDataView({ progress, onReplaceProgress, onNavigatePractice = () => {}, onOpenUnit = () => {}, onCompleteMilestone = () => {}, onAttempt = () => {} }) {
  ...
  const visibleMilestones = milestoneStates.filter((milestone) => milestone.passed || milestone.retentionDue || milestone.id === milestoneStates.find((candidate) => !candidate.passed)?.id);
  ...
  const followRecommendation = () => {
    ...
    if (recommendation.kind === "milestone") setActiveMilestone({ milestone: milestoneStates.find((item) => item.id === recommendation.milestoneId), kind: "stage" });
  };
  ...
  return (
    <div className="view-stack data-page">
      ...
      {activeMilestone && <MilestoneRunner milestone={activeMilestone.milestone} assessmentKind={activeMilestone.kind} onClose={() => setActiveMilestone(null)} onComplete={(id, scores, rating) => onCompleteMilestone(id, scores, rating, activeMilestone.kind)} onAttempt={onAttempt} />}
    </div>
  );
}
